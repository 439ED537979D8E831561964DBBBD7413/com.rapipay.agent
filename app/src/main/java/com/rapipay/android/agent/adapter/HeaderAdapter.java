...
public class HeaderAdapter extends RecyclerView.Adapter<HeaderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (mValues.get(position).getHeaderData().matches("([0-9]*[.])?[0-9]+") && !mValues.get(position).getHeaderValue().equalsIgnoreCase("Parent Mobile") && !mValues.get(position).getHeaderValue().equalsIgnoreCase("TOP_PARENT") && !mValues.get(position).getHeaderValue().equalsIgnoreCase("Agent Name"))
            holder.btn_name.setText(format(mValues.get(position).getHeaderData()));
        else if (mValues.get(position).getHeaderData().contains("~")) {
            String splits = mValues.get(position).getHeaderData().replace("~", "\n");
            holder.btn_name.setText(splits);
        } else
