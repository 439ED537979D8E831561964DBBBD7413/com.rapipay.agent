...
import com.rapipay.android.agent.adapter.NetworkTransferFragAdapter;
...
public class NetworkTransFragment extends Fragment implements RequestHandler {
    ...
    NetworkTransferFragAdapter adapter;
...
//        if (first == 1) {
            adapter = new NetworkTransferFragAdapter(getActivity(), list);
...
//            jsonObject.put("toIndex", String.valueOf(toIndex));
    ...
    private void customDialog_Ben(final NetworkTransferPozo pozo, String msg, final String type, String amount, String title) {
        ...
        if(type.equalsIgnoreCase("AMOUNTTRANSFER")) {
            ...
            btn_p_bank.setText("Current Balance : "+ pozo.getAgentBalance());
        ...
        final TextView text = (TextView) alertLayout.findViewById(R.id.input_amount_popup);
        ...
        btn_ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(type.equalsIgnoreCase("AMOUNTTRANSFER")) {
                    ...
                    if (!ImageUtils.commonAmount(text.getText().toString())) {
                        text.setError("Please enter valid data");
                        text.requestFocus();
                    }else {
                        customDialogConfirm(pozo, "Are you sure you want to Transfer?", "CONFIRMATION", text.getText().toString(), "Credit Confirmation");
                        alertDialog.dismiss();
                    }
                }else if(type.equalsIgnoreCase("NETWORK_CREDIT")) {
                    loadApi();
...
//        });
    ...
    public JSONObject getNetwork_Transfer(String receiverId, String txnAmount) {
        ...
        try {
            ...
            jsonObject.put("requestType", "BC_CHANNEL");
            jsonObject.put("typeMobileWeb", "mobile");
            jsonObject.put("transactionID", "CNC"+format.format(date));
            jsonObject.put("nodeAgentId", list.get(0).getMobilno());
            jsonObject.put("sessionRefNo", list.get(0).getAftersessionRefNo());
